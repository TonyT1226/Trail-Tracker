...
build_route.py        ->  data/trails/AT/route.json                      oriented, mile-scaled to the official length, simplified
fetch_osm_pois.py     ->  data/raw/osm_pois.geojson             shelters/gaps/towns from OpenStreetMap
build_anchors.py      ->  data/trails/AT/anchors.json                    snapped onto the route, giving each one a mile
...
- **Named points of interest** (the shelters, gaps, and towns in `data/trails/AT/anchors.json`): from OpenStreetMap, under the [ODbL](https://opendatacommons.org/licenses/odbl/) license, requiring attribution to "© OpenStreetMap contributors".
- **State boundaries** (`data/trails/AT/states.json`, `data/trails/AT/state_borders.geojson`): the US Census Bureau's Cartographic Boundary File, a US federal government work, in the public domain with no copyright restriction.
