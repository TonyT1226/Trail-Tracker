...
`build_states.py` 的 `--min-run` 控制"州界噪音"的平滑程度：AT 沿北卡/田纳西州界来回穿插约 200 英里、沿弗吉尼亚/西弗吉尼亚州界也有类似路段，默认 0.1 英里的平滑力度会在这些地方切出上百个碎片区间；调到 0.8 英里左右能把弗吉尼亚/西弗吉尼亚收敛到几段真实的跨州（在 Harpers Ferry 附近），北卡/田纳西那 200 英里本来就是真实的反复穿插，切多细都不算错，反正 `js/config.js` 里默认把这两州合并显示。

### 关于州界处的统计

北卡和田纳西之间约 200 英里，路线沿着州界来回穿插，弗吉尼亚和西弗吉尼亚之间也有类似的地方。这些地方按多边形划分会有噪声，所以默认把 NC 和 TN 合并显示（`js/config.js` 的 `stateGroups`）。
...
- 高程剖面图（用 Mapbox 地形取样，或在流水线里预先写入海拔）
- 跨设备自动同步（Supabase / Firebase，现在的导入导出是过渡方案）
- 记录里附照片或日志
